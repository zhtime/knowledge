...
由于B+Tree只有叶子节点保存key信息，查询任何key都要从root  走到叶子。所以B+Tree的查询效率更加稳定。
...
**在使用复合索引时遵循最左前缀法则，按照复合索引中的最左列索引来排序**
...
在满足第二范式的基础上，保证表格中的**字段不存在对非主键的传递依赖关系**。
...
根据上面描述的，将单个庞大的数据表单，分散到各个表中，这些还是在同一个库中，这就是**库内分表**。
